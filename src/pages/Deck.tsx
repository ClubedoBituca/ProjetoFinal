    ...
    <div className="min-h-screen bg-background">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          <Button variant="outline" size="icon" onClick={() => navigate(PATHS.DASHBOARD)} className="hover:bg-accent">
            <ArrowLeft className="w-4 h-4" />
          </Button>
          <div className="flex-1">
            <h1 className="text-4xl font-bold bg-magic-gradient bg-clip-text text-transparent">{deck.name}</h1>
            {deck.description && <p className="text-muted-foreground mt-1">{deck.description}</p>}
          </div>
          {deck.cards.length > 0 && (
            <Button onClick={handlePrintDeck} className="bg-blue-600 hover:bg-blue-700 text-white">
        ...
        {deck.cards.length === 0 ? (
          <Card className="mt-8">
            <CardContent className="py-16 text-center">
              ...
              <Button onClick={() => navigate(PATHS.HOME)}>Buscar Cartas</Button>
            </CardContent>
          </Card>
        ) : (
          <div className="mt-8">
            ...
            <Card className="mb-6 bg-muted/30 border-muted shadow-sm">
              ...
              <CardContent className="text-sm text-muted-foreground space-y-2">
                <ul className="list-disc pl-5 space-y-1">
                  <li>
                    Use papel <strong>couché fosco 180g ou 230g</strong>.
                  </li>
                  <li>
                    Configure a impressora para <strong>modo retrato</strong>, <strong>escala 100%</strong> e{" "}
                    <strong>margens padrão</strong>.
                  </li>
                  <li>
                    Após a impressão, recorte nas bordas e coloque as cartas em <strong>sleeves protetores</strong>.
                  </li>
                </ul>
              </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {deck.cards.map((deckCard) => (
                <DeckCardItem
                  ...
                  deckCard={deckCard}
                  onRemove={async () => await handleRemoveCard(deckCard.card.id)}
                />
              ))}
            </div>
          </div>
