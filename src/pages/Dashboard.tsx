  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-6">
        {/* Welcome Section */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">Bem vindo de volta, {user.username}!</h1>
          <p className="text-muted-foreground">Gerencie seus decks e coleção de Magic: The Gathering</p>
        </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{decks.reduce((total, deck) => total + getTotalCards(deck), 0)}</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-2xl font-bold">{decks.filter((deck) => getTotalCards(deck) >= 60).length}</p>
              ...
              <Card key={deck.id} className="group hover:shadow-lg transition-shadow">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    ...
                    <ConfirmDeleteDialog
                      actionText="Apagar"
                      onConfirm={async () => await deleteDeck(deck.id)}
                      title="Apagar Deck"
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between text-sm">
                      <span>Status:</span>
                      <span
                        className={`font-medium ${getTotalCards(deck) >= 60 ? "text-green-600" : "text-orange-600"}`}
                      >
                    ...
                    <div className="pt-3 space-y-2">
                      <Button asChild className="w-full">
                        <Link to={PATHS.DECK_DETAILS(deck.id)}>Editar Deck</Link>
                      </Button>
                    </div>
                  </div>
                </CardContent>
